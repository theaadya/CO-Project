...
opcode = {"add": "10000", "sub": "10001", "mul": "10110", "xor": "11010", "or": "11011", "and": "11100", "ls":"11001", "rs":"11000", "mov":"10010","div": "10111", "not": "11101", "cmp": "11110", "mov": "10011","st": "10100", "ld": "10101","jmp": "11111", "jlt": "01100", "je": "01111", "jgt": "01101", "hlt": "01010", "addf": "00000", "subf": "00001", "movf": "00010"}
lab_dic = {} 
vars = {}
vars_line = {}
machine_code = []
...
if flag == True:
    for i in range(len(inst_lst2)):
        ...
        if inst_lst2[i][0].lower() == "movf":
            flag_f = True
            if len(inst_lst2[i]) == 3 and inst_lst2[i][2][0] == "$":
                if inst_lst2[i][1].lower() in reg:
                    if "." in inst_lst2[i][2][1:] and str(float(inst_lst2[i][2][1:])) in floatNums.values():
                        val = list(floatNums.keys())[list(floatNums.values()).index(str(float(inst_lst2[i][2][1:])))]
                        op = "00010"
                        r1 = reg[inst_lst2[i][1].lower()]
                        machine_code.append(op + r1 + val)
                    else:
                        print(f'Error in line {i+1}: Invalid number for movf')
                        flag_f = False
                        break
                else:
                    print(f'Error in line {i+1}: Undefined Register name')
                ...
                print(f'Error in line {i+1}: Wrong Instruction syntax for movf')
                flag_f = False
                break
